...
POWER_COMPONENTS: dict[str, ComponentSpec] = {
    ...
    'Battery-LiPo-class': ComponentSpec(
        ...
        enclosure_relation='external',  # battery pack mounted external/accessible (matches snapshot SSOT)
    ),
    ...
    'Battery-4AA-class': ComponentSpec(
        name='4xAA Battery Holder (6V)', class_name='Battery-4AA-class',
        tags=['iface:passive', 'power:battery_alkaline'],
        mounting_holes=[],
        ports=[ConnectorPort(name='Wire-Exit', port_type='PWR', x=31.0, y=0.0, width=6.0, height=4.0, side='face')],
        enclosure_relation='external',  # battery pack mounted external/accessible
    ),
